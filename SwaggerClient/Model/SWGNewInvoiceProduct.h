#import <Foundation/Foundation.h>
#import "SWGObject.h"

/**
* Bind ERP API
* No description provided (generated by Swagger Codegen https://github.com/swagger-api/swagger-codegen)
*
* OpenAPI spec version: v1
* 
*
* NOTE: This class is auto generated by the swagger code generator program.
* https://github.com/swagger-api/swagger-codegen.git
* Do not edit the class manually.
*/





@protocol SWGNewInvoiceProduct
@end

@interface SWGNewInvoiceProduct : SWGObject


@property(nonatomic) NSString* _iD;

@property(nonatomic) NSNumber* price;

@property(nonatomic) NSNumber* vAT;

@property(nonatomic) NSNumber* indexNumber;

@property(nonatomic) NSNumber* iEPS;

@property(nonatomic) NSNumber* iEPSType;

@property(nonatomic) NSNumber* qty;

@property(nonatomic) NSString* comments;

@property(nonatomic) NSNumber* vATExempt;

@property(nonatomic) NSNumber* requireLot;

@property(nonatomic) NSArray<NSString*>* selectedImports;

@property(nonatomic) NSString* orderItemID;

@property(nonatomic) NSString* unit;

@end
