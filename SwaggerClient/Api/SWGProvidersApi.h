#import <Foundation/Foundation.h>
#import "SWGEditProvider.h"
#import "SWGNewProvider.h"
#import "SWGProviderListItemPage.h"
#import "SWGApi.h"

/**
* Bind ERP API
* No description provided (generated by Swagger Codegen https://github.com/swagger-api/swagger-codegen)
*
* OpenAPI spec version: v1.2
* 
*
* NOTE: This class is auto generated by the swagger code generator program.
* https://github.com/swagger-api/swagger-codegen.git
* Do not edit the class manually.
*/



@interface SWGProvidersApi: NSObject <SWGApi>

extern NSString* kSWGProvidersApiErrorDomain;
extern NSInteger kSWGProvidersApiMissingParamErrorCode;

-(instancetype) initWithApiClient:(SWGApiClient *)apiClient NS_DESIGNATED_INITIALIZER;

/// Agregar Proveedor
/// 
///
/// @param varNewProvider 
/// 
///  code:200 message:"OK"
///
/// @return NSString*
-(NSURLSessionTask*) providersAddProviderWithVarNewProvider: (SWGNewProvider*) varNewProvider
    completionHandler: (void (^)(NSString* output, NSError* error)) handler;


/// Borrar Proveedor
/// 
///
/// @param _id 
/// 
///  code:204 message:"No Content"
///
/// @return void
-(NSURLSessionTask*) providersDeleteByIDWithId: (NSString*) _id
    completionHandler: (void (^)(NSError* error)) handler;


/// Editar Proveedor
/// 
///
/// @param provider 
/// 
///  code:204 message:"No Content"
///
/// @return void
-(NSURLSessionTask*) providersEditProviderWithProvider: (SWGEditProvider*) provider
    completionHandler: (void (^)(NSError* error)) handler;


/// Obtiene la lista de proveedores.
/// 
///
/// @param filter Filters the results, based on a Boolean condition. (optional)
/// @param orderby Sorts the results. (optional)
/// @param top Returns only the first n results. (optional)
/// @param skip Skips the first n results. (optional)
/// 
///  code:200 message:"OK"
///
/// @return SWGProviderListItemPage*
-(NSURLSessionTask*) providersGetProvidersWithFilter: (NSString*) filter
    orderby: (NSString*) orderby
    top: (NSNumber*) top
    skip: (NSNumber*) skip
    completionHandler: (void (^)(SWGProviderListItemPage* output, NSError* error)) handler;



@end
