...
@interface SWGServicesApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation SWGServicesApi

NSString* kSWGServicesApiErrorDomain = @"SWGServicesApiErrorDomain";
NSInteger kSWGServicesApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) servicesAddProductWithVarNewService: (SWGNewService*) varNewService
    ...
    // verify the required parameter 'varNewService' is set
    if (varNewService == nil) {
        NSParameterAssert(varNewService);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"varNewService"] };
            NSError* error = [NSError errorWithDomain:kSWGServicesApiErrorDomain code:kSWGServicesApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/api/Services"];
...
-(NSURLSessionTask*) servicesDeleteByIDWithId: (NSString*) _id
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/api/Services/{id}"];
...
-(NSURLSessionTask*) servicesGetServicesWithFilter: (NSString*) filter
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"SWGServicesPage*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((SWGServicesPage*)data, error);
                                }
                            }];
}
