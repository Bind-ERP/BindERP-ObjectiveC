...
NSString* kSWGOrdersApiErrorDomain = @"SWGOrdersApiErrorDomain";
NSInteger kSWGOrdersApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) ordersAddOrderWithVarNewOrder: (SWGNewOrder*) varNewOrder
    ...
    // verify the required parameter 'varNewOrder' is set
    if (varNewOrder == nil) {
        NSParameterAssert(varNewOrder);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"varNewOrder"] };
            NSError* error = [NSError errorWithDomain:kSWGOrdersApiErrorDomain code:kSWGOrdersApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/api/Orders"];
...
-(NSURLSessionTask*) ordersDeleteOrderWithId: (NSString*) _id
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/api/Orders/{id}"];
...
-(NSURLSessionTask*) ordersGetByIDWithId: (NSString*) _id
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"SWGOrderDetails*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((SWGOrderDetails*)data, error);
                                }
                            }];
}
