...
@interface SWGProspectsApi ()

@property (nonatomic, strong, readwrite) NSMutableDictionary *mutableDefaultHeaders;

@end

@implementation SWGProspectsApi

NSString* kSWGProspectsApiErrorDomain = @"SWGProspectsApiErrorDomain";
NSInteger kSWGProspectsApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) prospectsAddProspectWithVarNewProspect: (SWGNewProspect*) varNewProspect
    ...
    if (varNewProspect == nil) {
        NSParameterAssert(varNewProspect);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"varNewProspect"] };
            NSError* error = [NSError errorWithDomain:kSWGProspectsApiErrorDomain code:kSWGProspectsApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    }

    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/api/Prospects"];
...
-(NSURLSessionTask*) prospectsDeleteByIDWithId: (NSString*) _id
    ...
    NSMutableString* resourcePath = [NSMutableString stringWithFormat:@"/api/Prospects/{id}"];
...
-(NSURLSessionTask*) prospectsEditClientWithProspect: (SWGEditProspect*) prospect
    ...
    if (prospect == nil) {
        NSParameterAssert(prospect);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"prospect"] };
            NSError* error = [NSError errorWithDomain:kSWGProspectsApiErrorDomain code:kSWGProspectsApiMissingParamErrorCode userInfo:userInfo];
            handler(error);
        }
        return nil;
...
-(NSURLSessionTask*) prospectsGetWithId: (NSString*) _id
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"SWGProspectDetails*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((SWGProspectDetails*)data, error);
                                }
                            }];
}
...
///
///  @returns SWGProspectListItemPage*
///
-(NSURLSessionTask*) prospectsGetProspectsWithFilter: (NSString*) filter
    ...
    return [self.apiClient requestWithPath: resourcePath
                        ...
                        requestContentType: requestContentType
                       responseContentType: responseContentType
                              responseType: @"SWGProspectListItemPage*"
                           completionBlock: ^(id data, NSError *error) {
                                if(handler) {
                                    handler((SWGProspectListItemPage*)data, error);
                                }
                            }];
}
